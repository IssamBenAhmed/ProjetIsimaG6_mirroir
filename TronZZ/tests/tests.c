/**
 * @file tests.c
 * @brief Suite de tests unitaires pour valider les composants critiques de TronZZ.
 *
 * Ce fichier contient les tests unitaires essentiels demandés pour valider le moteur physique,
 * la perception, le calcul de probabilités Softmax et la descente de gradient stochastique (REINFORCE).
 */
...
/* Accès à la matrice globale de poids theta de l'agent pour vérification directe */
...
/**
 * @brief Test de la fonction cause_mort (détection des collisions hors-grille et obstacles).
 */
...
void test_generer_phi(void) {
    ...
    assert(fabsf(phi[11] - (0.5f * 0.2f)) < EPSILON); // distances[0] * distances[1]
    assert(fabsf(phi[12] - (0.3f * 0.9f)) < EPSILON); // densite[2] * densite[3]
    assert(fabsf(phi[13] - (0.5f * 0.4f)) < EPSILON); // distances[0] * densite[1]
    assert(fabsf(phi[14] - (0.8f * 0.2f)) < EPSILON); // distances[2] * distances[1]
...
void test_choisir_action(void) {
    ...
    float somme_probabilites = frame.probabilites[0] + frame.probabilites[1] + frame.probabilites[2];
    ...
    assert(fabsf(frame.recompense - 1.0f) < EPSILON); // Récompense par défaut de survie

    printf("  [OK] test_choisir_action\n");
}

/**
 * @brief Test mathématique de la mise à jour stochastique du gradient (REINFORCE).
 */
void test_maj_theta(void) {
    // Étape 1 : Réinitialisation forcée de theta à 0.5 pour rendre le calcul déterministe et prédictible
    for (int i = 0; i < 15; i++) {
        for (int j = 0; j < 3; j++) {
            theta[i][j] = 0.5f;
        }
    ...
    for (int i = 0; i < 15; i++) ep.frames[0].phi[i] = 0.1f;
    ...
    for (int i = 0; i < 15; i++) ep.frames[1].phi[i] = 0.2f;
    ...
    //   Correction sur theta : theta_new[i][j] = theta_old[i][j] + alpha * phi_1[i] * (y_j - pi_j) * G_1
    //   y_0 = 1, erreur_0 = 1.0 - 0.5 = 0.5 -> delta_theta[i][0] = 0.0001 * 0.2 * 0.5 * (-100) = -0.001f
    //   y_1 = 0, erreur_1 = 0.0 - 0.2 = -0.2 -> delta_theta[i][1] = 0.0001 * 0.2 * (-0.2) * (-100) = 0.0004f
    //   y_2 = 0, erreur_2 = 0.0 - 0.3 = -0.3 -> delta_theta[i][2] = 0.0001 * 0.2 * (-0.3) * (-100) = 0.0006f
    //
    // A t = 0 :
    //   G_0 = R_0 + gamma * G_1 = 1.0f + 0.99f * (-100.0f) = -98.0f.
    //   Correction sur theta : theta_new[i][j] += alpha * phi_0[i] * (y_j - pi_j) * G_0
    //   y_0 = 0, erreur_0 = 0.0 - 0.3 = -0.3 -> delta_theta[i][0] += 0.0001 * 0.1 * (-0.3) * (-98.0) = 0.000294f
    //   y_1 = 1, erreur_1 = 1.0 - 0.4 = 0.6 -> delta_theta[i][1] += 0.0001 * 0.1 * (0.6) * (-98.0) = -0.000588f
    //   y_2 = 0, erreur_2 = 0.0 - 0.3 = -0.3 -> delta_theta[i][2] += 0.0001 * 0.1 * (-0.3) * (-98.0) = 0.000294f
    //
