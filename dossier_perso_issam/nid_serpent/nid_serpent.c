...
int main(int argc, char* argv[]) {
    ...
    SDL_Window* window = SDL_CreateWindow("Serpent Fluide", 
    ...
    // activation de la vsync pour une animation fluide callee sur le taux de rafraichissement
    ...
    while(isRunning) {
        ...
        // cela rajoute une oscillation (dandinement) continue autour de l'angle de base
        float head_angle = base_angle + sin(SDL_GetTicks() * 0.005f) * 0.8f; 

        // conversion des coordonnees polaires (angle, vitesse) vers cartesiennes (x, y)
        // la trigonometrie classique: l'axe x depend du cosinus, l'axe y depend du sinus
        ...
        // de cette facon la tete est affichee en dernier et recouvre le reste du corps
        for(int i = SNAKE_LENGTH - 1; i >= 0; i--) {
            
            // on centre le carre sur les coordonnees du segment
            SDL_Rect rect = {
                (int)(snake[i].x - RECT_SIZE / 2),
                (int)(snake[i].y - RECT_SIZE / 2),
                RECT_SIZE, RECT_SIZE
            };
