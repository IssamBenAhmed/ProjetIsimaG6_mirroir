...
int main(int argc, char *argv[]) {
    ...
    SDL_Window *window = SDL_CreateWindow(
        "Jeu de la Vie - Architecture SMA",
    ...
    // initialisation de la double grille matrice pour le swap
    ...
    int cam_y = 0;
    bool running = true;
    bool paused = true; // demarre en pause pour pouvoir dessiner au clic initial
    ...
    while (running) {
        ...
        if (!paused && (current_time - last_update_time >= DELAY_MS)) {
            ...
            // pointeur ou index swap en o constant
            courant = 1 - courant;
            futur = 1 - futur;
