 ...
 */
static void end_sdl(bool ok, const char *message, SDL_Window *window,
...
/**
 * @brief charge la feuille de sprite et deplace l'image affichee selon les
 * fleches.
 *
 * @param image_path chemin vers la feuille de sprite.
 * @param background_pic chemin vers le fond, ou NULL si aucun fond n'est
 * souhaite.
 */
static void display_movement(const char *image_path,
  ...
  window = SDL_CreateWindow(image_path, SDL_WINDOWPOS_CENTERED,
                            SDL_WINDOWPOS_CENTERED, WINDOW_WIDTH, WINDOW_HEIGHT,
                            SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
  if (window == NULL) {
    end_sdl(false, "erreur ouverture window", window, renderer, texture,
            background_texture);
  }
  ...
  if (renderer == NULL) {
    end_sdl(false, "erreur init renderer", window, renderer, texture,
            background_texture);
  }
  ...
  if (texture == NULL) {
    end_sdl(false, IMG_GetError(), window, renderer, texture,
            background_texture);
  }
  ...
  if (SDL_QueryTexture(texture, NULL, NULL, &image_width, &image_height) != 0) {
    end_sdl(false, SDL_GetError(), window, renderer, texture,
            background_texture);
  }
  ...
  // ordre des lignes dans la feuille : 0 = haut, 1 = gauche, 2 = bas, 3 =
  // droite.
  ...
  SDL_RenderCopy(renderer, texture, &tab_src[direction_index][k],
                 &destination_rect);
  ...
  while (is_running) {
    ...
    while (SDL_PollEvent(&event)) {
      ...
      case SDLK_UP:
        // deplacement vers le haut et lecture de la ligne d'animation
        // correspondante.
