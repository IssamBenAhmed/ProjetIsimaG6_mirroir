 ...
 * -----------------------------------*/
int random_int_thread(uint32_t *seed,
                      int max) { // seed aléatoire pour entrainement parallèle
  if (max <= 0)
    return 0;
...
void initialiser_partie(int grille[WIDTH][HEIGHT],
  ...
  while (k < 4) {
    ...
    if (grille[x][y] != CELL_EMPTY) { /* quand la case est deja remplie, on
                                         refait son positionnement */
      continue;
    } else {
      grille[x][y] = player_list[k]; /* sinon, on la situe */
      pos_motos[k + 1][0] =
          x; /* on parametre la position initiale de la moto */
      pos_motos[k + 1][1] = y;
      etats_vie[k + 1] = true; /* elle vit */
      dir_motos[k + 1] =
          DIR_UP; /* direction initiale (possiblement aleatoire) */
      k++;
    }
  }
}
...
int trouver_gagnant(bool etats_vie[MAX_MOTOS + 1]) {
  int moto_vivant =
      compter_moto_vivants(etats_vie); /* compter le nombre de motos vivantes */
  if (moto_vivant == 2) {
    for (int i = 1; i < (MAX_MOTOS + 1);
         i++) { /* on ne rend pas compte de la case vide qui correspond a
                   etat_vie[0] */
      if (etats_vie[i] != true) {
        continue;
      } else {
...
bool gagnant_ou_pas(bool etats_vie[MAX_MOTOS + 1],
  ...
  if (gagner_ou_pas != -1) {
    ...
    /* correction gahui : on utilise taille - 1 car l'index de memoire a deja
     * avance */
    modifier_recompense(
        100.0f,
...
int cause_mort(int grille[WIDTH][HEIGHT], int x, int y) {
  if (x < 0 || x >= WIDTH || y < 0 ||
      y >= HEIGHT) { /* elle a tape le mur : cas peu probable */
    return -1;
  }
  return grille[x][y]; /* renvoie le trajet que la moto a heurte et qui a cause
                          sa mort */
}

void modifier_recompense(
    float recompense,
    FrameMemoire *frame) { /* donner la recompense par rapport a l'action
                              realisee par l'agent */
  frame->recompense += recompense;
}

/*---------------------- la partie pour calculer la perception de la
 * moto---------------------------------*/
static void assigner_zone_adversaire(int id_trouve, int zone, int mon_id,
  ...
  for (int k = CELL_PLAYER; k <= CELL_AI_3; k++) {
    if (k == mon_id)
      continue; /* on ignore sa propre moto */
    if (k == id_trouve) {
      if (slot == 1)
        *z1 = zone;
      else if (slot == 2)
...
void mettre_a_jour_monde(int grille[WIDTH][HEIGHT],
  ...
  for (int i = CELL_PLAYER; i <= CELL_AI_3; i++) {
    ...
    /* seules les agents (id >= 2) utilisent la perception et ta fonction
     * choisir_action */
    if (i >= CELL_AI_1) {
      Perception perception;
      calculer_perception(grille, x, y, dir_motos[i], i, &perception);

      /* correction gahui : ici on pointe vers [taille] (la case vide). pas de
       * -1 sinon crash memoire des la frame 0 ! */
      int action =
          choisir_action(perception, &memoires[i].frames[memoires[i].taille]);
      ...
      memoires[i].taille++; /* l'enregistrement est termine, on deplace le
                               pointeur de taille */
    }
    ...
    if (nx >= 0 && nx < WIDTH && ny >= 0 && ny < HEIGHT &&
      ...
      /* correction gahui : on utilise taille - 1 pour modifier la frame qu'on a
       * valide juste au dessus */
      ...
      if (cause == CELL_EMPTY) { // si le moto est mort par le mur, il ne dépend
                                 // pas de récompense
        continue;
      }
      if (cause ==
          CELL_PLAYER) { // cas exception si le joueur est mort par lui-meme
...
void mettre_a_jour_monde_entrainement(int grille[WIDTH][HEIGHT],
  ...
  for (int i = CELL_PLAYER; i <= CELL_AI_3;
       i++) { /* ici, le joueur est remplace par un agent */
    if (!etats_vie[i])
      continue;
