...
SDL_Window *creer_fenetre(int largeur, int hauteur) {
  ...
  window = SDL_CreateWindow("Jeu de la Vie - SMA", SDL_WINDOWPOS_CENTERED,
...
void gerer_interaction(SDL_Event *event, int grille[WIDTH][HEIGHT], float *zoom,
  ...
  if (event->type == SDL_MOUSEBUTTONDOWN) {
    ...
    if (event->button.button == SDL_BUTTON_LEFT) {
      ...
      /* on memorise la position x de depart pour calculer ensuite le
       * deplacement */
      ancien_x = souris_x;

      /* on memorise la position y de depart pour calculer ensuite le
       * deplacement */
      ancien_y = souris_y;

      /* calcule la cellule visee pour un toggle possible au relachement */

      /* on convertit la position souris en position locale dans la grille selon
       * la camera x */
  ...
  if (event->type == SDL_MOUSEBUTTONUP) {
    ...
    if (event->button.button == SDL_BUTTON_LEFT) {
      ...
      if (!clic_gauche_en_drag) {
        ...
        /* on enleve le decalage de la camera pour obtenir la position dans la
         * grille */
        local_x = souris_x - *cam_x;
        ...
        if (local_x >= 0 && local_y >= 0) {
          ...
          /* on verifie que la cellule cliquee est bien dans les limites de la
           * grille */
          if (case_x >= 0 && case_x < WIDTH && case_y >= 0 && case_y < HEIGHT) {
  ...
  if (event->type == SDL_MOUSEMOTION) {
    ...
    if (clic_gauche_appuye) {
      ...
      /* xrel et yrel indiquent le deplacement relatif de la souris depuis le
       * dernier mouvement */
      if (event->motion.xrel != 0 || event->motion.yrel != 0) {
        /* la souris a bouge, donc ce n'est plus un clic simple mais un
         * deplacement */
      ...
      /* on modifie le decalage horizontal de la camera selon le mouvement de la
       * souris */
      *cam_x = *cam_x + event->motion.x - ancien_x;

      /* on modifie le decalage vertical de la camera selon le mouvement de la
       * souris */
      *cam_y = *cam_y + event->motion.y - ancien_y;
      ...
      /* on memorise la nouvelle position x de la souris pour le prochain
       * mouvement */
      ancien_x = event->motion.x;

      /* on memorise la nouvelle position y de la souris pour le prochain
       * mouvement */
