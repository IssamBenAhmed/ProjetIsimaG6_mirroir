...
int main(int argc, char *argv[]) {
  ...
  SDL_Window *window = SDL_CreateWindow(
      "smatron - sma no reinforce", SDL_WINDOWPOS_CENTERED,
  ...
  while (running) {
    ...
    // Capture des événements gérée par Nabil
    ...
    if (now - last_tick >= 2 * FRAME_DELAY_MS) {
      ...
      // Vérification de la condition de victoire/fin (1 seule moto restante ou
      // moins)
