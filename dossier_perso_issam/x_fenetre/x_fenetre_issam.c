...
int main(int argc, char* argv[]) {
    ...
    SDL_Window* mes_fenetres[NB_FENETRES_PAR_DIAG * 2]; // init du tableau de fenetre à générer
    int index = 0;

    // 1. Génération de la première diagonale
    for (int i = 0; i < NB_FENETRES_PAR_DIAG; i++) {
        int pos_x = i * (largeur_ecran - TAILLE_FENETRE) / (NB_FENETRES_PAR_DIAG - 1);
        int pos_y = i * (hauteur_ecran - TAILLE_FENETRE) / (NB_FENETRES_PAR_DIAG - 1);

        mes_fenetres[index] = SDL_CreateWindow("Diagonale 1", pos_x, pos_y, TAILLE_FENETRE, TAILLE_FENETRE, SDL_WINDOW_SHOWN);
    ...
    for (int i = 0; i < NB_FENETRES_PAR_DIAG; i++) {
        int pos_x = (largeur_ecran - TAILLE_FENETRE) - (i * (largeur_ecran - TAILLE_FENETRE) / (NB_FENETRES_PAR_DIAG - 1));
        int pos_y = i * (hauteur_ecran - TAILLE_FENETRE) / (NB_FENETRES_PAR_DIAG - 1);

        mes_fenetres[index] = SDL_CreateWindow("Diagonale 2", pos_x, pos_y, TAILLE_FENETRE, TAILLE_FENETRE, SDL_WINDOW_SHOWN);
        SDL_Delay(50);
        index++;
    }
