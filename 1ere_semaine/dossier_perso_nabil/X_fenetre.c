...
#define LARGEUR_MIN                                                            \
  70 // Largeur minimal, Quand on reduit avec r, on ne descend pas en dessous de
     // cette valeurs.
#define HAUTEUR_MIN                                                            \
  50 // Hauteur minimal, Quand on reduit avec r, on ne descend pas en dessous de
     // cette valeurs.

#define MARGE                                                                  \
  40 // Marge entre le X et les bords de l'ecran.Cela evite de coller les
     // fenetres au bord de l'ecran.

#define DEPLACEMENT                                                            \
  20 // Nombre de pixels utilises pour deplacer les fenetres avec les fleches.
...
int main(void) {
  ...
  SDL_Window *fenetres[NB_FENETRES] = {
      NULL}; // tableau de 29 fenetres qui ne sont pas encore crées.
  ...
  SDL_DisplayMode
      screen; // screen contient la taille de l'écran(largeur et hauteur).
  ...
  for (i = 0; i < NB_FENETRES; i++) {
    ...
    fenetres[i] = SDL_CreateWindow(
        titre, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, LARGEUR_INIT,
        HAUTEUR_INIT,
        SDL_WINDOW_SHOWN |
            SDL_WINDOW_RESIZABLE // fenetres visibles et redimensionnées.
    );
  ...
  for (i = 0; i < NB_PAR_DIAGONALE; i++) // premiére diagonale.
  {
    ...
    SDL_SetWindowPosition(fenetres[k], x,
                          y); // Placer la fenetre k à la position calculée.
  ...
  for (i = 0; i < NB_PAR_DIAGONALE; i++) // deuxiéme diagonale.
  {
    if (i ==
        NB_PAR_DIAGONALE /
            2) // on saut le milieu sinon on aurait deux fenetres au centre.
    {
      continue;
    }
