...
void play_with_texture_4(SDL_Texture* texture,SDL_Window* window,SDL_Renderer* renderer)
{
    ...
    SDL_GetWindowSize(window, &window_dim.w, &window_dim.h); // Rectangle définissant la fenêtre

    int tex_w, tex_h;
    SDL_QueryTexture(texture, NULL, NULL, &tex_w, &tex_h);// Rectangle définissant la zone de la texture à récupérer

    int frame_count = 8; //frame de animation à utiliser, en fonction de l'image
    int source_w = tex_w / frame_count;
    int source_h = tex_h/4;

    SDL_Rect source = {0, 0, source_w, source_h};  // Rectangle définissant la zone de la texture à récupérer

    SDL_Rect destination = {// Rectangle définissant où la zone_source doit être déposée dans le renderer
        0,
        (window_dim.h - source_h * 2) / 2,
        source_w * 2,
        source_h * 2
    };
    ...
    for (int i = 0; i < nb_it; i++)
    {
        ...
        source.x = (i / 5) % frame_count * source_w; //on déplace la zone de source à utiliser
...
void play_with_texture_3_alter(SDL_Texture* my_texture,SDL_Window* window,SDL_Renderer* renderer) {
    SDL_Rect 
    ...
    for (int i = 0; i < nb_it; ++i) { //mouvement cercle
        ...
        destination.x = (window_dimensions.w / 2)+ radius * cos(angle)- destination.w / 2;

        destination.y = (window_dimensions.h / 2)+ radius * sin(angle)- destination.h / 2;
      ...
      SDL_SetTextureAlphaMod(my_texture,((float)i / nb_it) * 255);     // L'opacité va passer de 0 à 255 au fil de l'animation
